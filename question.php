...
class qtype_manip_question extends question_graded_automatically {
    ...
    public $regex;
    public $gabarit; // à confirmer ... FILE ID? à ajouter à la DB ?
    ...
    public function summarise_response(array $response) {
        if (!array_key_exists('answer', $response)) {
            ...
            return get_string('true', 'qtype_manip');
        } else {
            return get_string('false', 'qtype_manip');
        }
    }
    ...
    public function classify_response(array $response) {
        ...
        if ($response['answer']) {
            return array($this->id => new question_classified_response(1,
                    get_string('true', 'qtype_manip'), $fraction));
        } else {
            return array($this->id => new question_classified_response(0,
                    get_string('false', 'qtype_manip'), $fraction));
        }
    }
    ...
    public function grade_response(array $response) {
        // TODO: manip - faire l'appel au code regex de vérification du fichier
